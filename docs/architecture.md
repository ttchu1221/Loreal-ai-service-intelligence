...
目标主链路采用 agent-first：渠道或 CRM 在消费者进线时把聊天、订单和历史工单的最小必要快照提交
到 `/v1/agent/intakes`。编排层生成统一服务轨迹和 AI 坐席辅助包，人工客服审核回复草稿并执行动作，
Ticket 继续记录回复、动作完成、消费者确认和重开。消费者机器人链路作为兼容入口存在，不再作为
目标架构的唯一主流程。
